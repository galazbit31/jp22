/*
  # Fix affiliate permissions

  1. Security
    - Make affiliate permissions fully permissive
    - Allow anonymous operations for all affiliate collections
*/

-- Drop existing policies for affiliate tables
DROP POLICY IF EXISTS "Anyone can manage affiliate_referrals" ON public.affiliate_referrals;
DROP POLICY IF EXISTS "Anyone can manage affiliate_commissions" ON public.affiliate_commissions;
DROP POLICY IF EXISTS "Anyone can manage affiliate_settings" ON public.affiliate_settings;
DROP POLICY IF EXISTS "Anyone can manage affiliate_payouts" ON public.affiliate_payouts;
DROP POLICY IF EXISTS "Anyone can manage affiliates" ON public.affiliates;

-- Create fully permissive policies for all affiliate tables
CREATE POLICY "Full access to affiliate_referrals"
ON public.affiliate_referrals FOR ALL
USING (true)
WITH CHECK (true);

CREATE POLICY "Full access to affiliate_commissions"
ON public.affiliate_commissions FOR ALL
USING (true)
WITH CHECK (true);

CREATE POLICY "Full access to affiliate_settings"
...
CREATE POLICY "Full access to affiliates"
